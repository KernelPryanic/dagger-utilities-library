...
from dul.modules import filesystem
...
async def docs(path: str, check: bool):
    ...
    if check:
        ...
        if not os.path.exists(readme):
            log.error(f"File {readme} doesn't exist", hint=f'Try running "{args.command}" in "{path}"')
            raise SystemExit(1)
        else:
            ...
            if current_readme != new_readme:
                log.error(
                    f"File {readme} doesn't seem to be up-to-date.", hint=f'Try running "{args.command}" in "{path}"'
                )
                raise SystemExit(1)
    else:
        generate_readme()
