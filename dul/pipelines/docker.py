...
class cli(pipe):
    def __init__(
        ...
        self.cli = ["docker"] + self.schema.process(parameters) + extra_args
    ...
    def build(
        ...
        disable_content_trust: bool = None,
        file: str = None,
        iidfile: str = None,
        isolation: str = None,
    ...
    class __login(pipe):
        def __init__(
            self, parent: pipe, username: str = None, password: str = None, server: str = None, extra_args: list = []
        ) -> pipe:
            parameters = locals()
            self.schema = Schema(
                {
            ...
            self.cli = parent.cli + ["login"] + extra_args + self.schema.process(parameters)

        def azure(
            self, client_id: str, client_secret: str, tenant_id: str, cloud_name: str = None, extra_args: list = []
        ) -> pipe:
            self.cli += ["azure"] + self.schema.process(locals()) + extra_args
            return self

    def login(self, server: str = None, username: str = None, password: str = None, extra_args: list = []) -> __login:
        parameters = locals()
        parameters.pop("self")
    ...
    def __call__(self, client: Client, container: Container, root: str = None) -> Container:
        return pipe.__call__(
            self,
            container.with_unix_socket("/var/run/docker.sock", client.host().unix_socket("/var/run/docker.sock")),
            root,
        )
